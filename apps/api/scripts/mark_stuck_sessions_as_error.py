...
Defesa em profundidade contra o cenário em que o worker morre de forma
catastrófica antes de qualquer ``except`` rodar (OOM kill do Cloud Run,
crash do entrypoint, etc) e a sessão fica em ``status='processing'``
pra sempre.

O caminho normal já está coberto:
    - ``WorkerSettings.job_timeout=900s`` → ARQ cancela após 15min.
    - ``except asyncio.CancelledError`` em ``run_reconciliation_processing``
      marca a sessão como ``error`` antes de propagar.
...
que ``job_timeout`` pra não colidir com jobs lentos legítimos.
...
# Margem confortável acima do ``job_timeout`` do worker (900s = 15min). 25min
# evita marcar como stuck um job que está só processando lentamente.
