...
class ExtractedTransaction(BaseModel):
    ...
    # anterior. A regra 4 do prompt manda extrair TODA movimentação visível, e
    # numa fatura de cartão o pagamento da fatura anterior aparece — então a IA
    # o inclui. Não excluímos (perderia o dado); MARCAMOS, para que o checksum
    # do cartão some tudo EXCETO os pagamentos e feche no total da fatura.
    # Default False: só faturas de cartão têm pagamento a marcar; conta corrente
    # nunca marca. Chave ausente no tool_use = False.
    is_payment: bool = Field(
        default=False,
        description=(
            "True apenas para linhas de PAGAMENTO da fatura anterior (cartão). "
            "Excluídas do checksum do cartão. Conta corrente: sempre False."
        ),
    )
